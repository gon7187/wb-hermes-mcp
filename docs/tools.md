...
| `wb_plan_update_minus_phrases` | Минус-фразы: по умолчанию `mode=add` — при применении выгружает текущие, дописывает новые, отправляет полный список и проверяет обратным чтением; `remove` — убрать; `replace` — заменить целиком. |
| `wb_audit_campaigns` | Аудит РК за период: какой канал оставить (поиск / реки+каталог), кластеры-кандидаты в минус, слабые РК. Только чтение. |
| `wb_plan_update_placements` | Вкл/выкл зон показа (поиск+каталог / рекомендации) у CPM-кампаний с ручной ставкой, до 50 за вызов. |
...
| `wb_apply_changes` | Применяет до 50 планов за один вызов (по порядку, ~1 с между записями). |
